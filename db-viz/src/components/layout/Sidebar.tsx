...
export default function Sidebar({
  ...
  return (
    <motion.aside
      initial={false}
      animate={{ 
        width: isCollapsed ? 72 : (typeof window !== 'undefined' && window.innerWidth >= 1024 ? 256 : 240),
        opacity: 1
      }}
    ...
    >
      ...
      <div className={`p-4 border-b ${theme?.navbar?.includes('slate') ? 'border-slate-700 bg-slate-800' : 'border-gray-200/80 bg-gray-50/80'}`}>
        <div className={`flex items-center ${isCollapsed ? 'justify-center' : 'justify-between'} ${isCollapsed ? 'mb-0' : 'mb-3'}`}>
          ...
          >
            <FolderPlus className={isCollapsed ? "w-5 h-5" : "w-4 h-4"} />
          </motion.button>
        </div>
              ...
              return (
                <div key={db.id}>
                  ...
                  >
                    ...
                    <Database className={`${isCollapsed ? 'w-6 h-6' : 'w-3.5 h-3.5'} ${isSelected ? (theme?.text || 'text-gray-900') : (theme?.textSecondary || 'text-gray-600')}`} />
                    {!isCollapsed && (
                      <>
      ...
      <div className={`mt-auto border-t ${theme?.navbar?.includes('slate') ? 'border-slate-700' : 'border-gray-200/80'}`}>
        ...
        {showModeButtons && (
          ...
          >
            <Terminal className={isCollapsed ? "w-5 h-5" : "w-4 h-4"} />
        ...
        {showModeButtons && (
          ...
          >
            <Presentation className={isCollapsed ? "w-5 h-5" : "w-4 h-4"} />
          ...
          <div className={`border-t ${theme?.navbar?.includes('slate') ? 'border-slate-700' : 'border-gray-200/80'}`}>
            ...
            >
              {user.photoURL ? (
                ...
                <div className={`rounded-full ${theme?.button || 'bg-gray-900'} flex items-center justify-center ${isCollapsed ? 'w-6 h-6' : 'w-8 h-8'}`}>
                  <User className={`${isCollapsed ? 'w-3 h-3' : 'w-4 h-4'} text-white`} />
                </div>
              )}
              {!isCollapsed && (
                <>
